...
if (! defined('DIAFAN'))
{
	echo 'DIAFAN.CMS version 6.0.5.1'; exit;
}
...
class Init extends Core
{
	...
	private function include_other_file()
	{
		...
		if (! ROUTE_END && preg_match('/^\/'.(REVATIVE_PATH ? REVATIVE_PATH.'\/' : '').'(.*)\/$/', $_SERVER["REQUEST_URI"], $m))
		{
		...
		if(MOBILE_VERSION)
		{
			...
			if(! $is_mobile && empty($_SESSION["mobile_no"]))
			{
				...
				if($detect->isMobile())
				{
					...
					$this->redirect(BASE_PATH.($lang ? $lang.'/' : '').'m/'.preg_replace('/^\/'.(REVATIVE_PATH ? preg_quote(REVATIVE_PATH, '/').'\/' : '').($lang ? preg_quote($lang, '/').'(\/)*' : '').'/', '', getenv('REQUEST_URI')), 301);
				}
			}
		}
		...
		if ($_GET["rewrite"] || defined('_LANG') && _LANG)
		{
			...
			foreach ($this->_languages->all as $language)
			{
				if (! defined('_LANG') && $rew[0] == $language["shortname"] || defined('_LANG') && $language["id"] == _LANG)
				{
					...
					$this->mobile_version();
					define('BASE_PATH_HREF', BASE_PATH.(! $language["base_site"] ? $language["shortname"].'/' : '').(IS_MOBILE ? 'm/' : ''));
					define('BASE_URL', ($domain ? $domain : getenv("HTTP_HOST")).(REVATIVE_PATH ? '/'.REVATIVE_PATH : '').(! $language["base_site"] ? '/'.$language["shortname"] : '').(IS_MOBILE ? '/m' : ''));
		...
		if (! defined('_LANG'))
		{
			foreach ($this->_languages->all as $row)
			{
				if ($row["base_site"])
				{
					...
					define('BASE_PATH_HREF', BASE_PATH.(IS_MOBILE ? 'm/' : ''));
					define('BASE_URL', ($domain ? $domain : getenv("HTTP_HOST")).(REVATIVE_PATH ? '/'.REVATIVE_PATH : '').(IS_MOBILE ? '/m' : ''));
					break;
				}
			}
		}
