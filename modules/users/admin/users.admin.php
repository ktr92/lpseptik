...
class Users_admin extends Frame_admin
{
	...
	public function list_variable_parent($row, $var)
	{
		...
		{
			$this->cache["prepare"]["session"] = DB::query_fetch_key_value("SELECT user_id, session_id FROM {sessions} WHERE timestamp>=%d AND user_id IN (%s)", (time() - 900), implode(",", $this->diafan->rows_id), "user_id", "session_id");
		}
		$text = '<div style="display: inline-block;">';
		if(! empty($this->cache["prepare"]["session"][$row["id"]]))
		{
			$text .= ' <span style="color: #ffffff; background-color: red; padding: 0px 5px; margin-left: 10px;">'.$this->diafan->_('на сайте').'</span>';
		}
		$text .= '</div>';
		return $text;
	}
	...
	public function save_variable_phone()
	{
		...
		{
			DB::query("INSERT INTO {subscribtion_phones} (phone, created, name, act) VALUES ('%s', %d, '%h', '1')", $phone, $_POST["fio"], time());
		}
