...
class Cart_action extends Action 
{
	...
	public function recalc()
	{
		...
		$_SESSION["cart_delivery_summ"] = 	$this->diafan->filter($_POST, 'float', 'delivery_summ');
		$_SESSION["cart_delivery_info"] = nl2br($this->diafan->filter($_POST, 'string', 'delivery_info'));
	...
	public function order()
	{
		...
		{
			$delivery_summ_empty = false;
			$row = DB::query_fetch_array("SELECT * FROM {shop_delivery} WHERE [act]='1' AND trash='0'AND id=%d LIMIT 1", $_SESSION["cart_delivery"]);
			...
			if($row["service"])
			{
				...
				if($threshold)
				{
					...
					{
						$delivery_summ_empty = true;
					}
				}
				if(! $delivery_summ_empty && empty($_SESSION["cart_delivery_summ"]))
				{
					$this->result["errors"][0] = $this->diafan->_('Стоимость доставки не определена. Пожалуйста, задайте дополнительные опции.', false);
				}
			}
		}
		...
		if(! empty($_SESSION["cart_delivery"]))
		{
			if($row = DB::query_fetch_array("SELECT * FROM {shop_delivery} WHERE [act]='1' AND trash='0'AND id=%d LIMIT 1", $_SESSION["cart_delivery"]))
			{
				if($row["service"])
				{
					$delivery_info = ! empty($_SESSION["cart_delivery_info"]) ? $_SESSION["cart_delivery_info"] : '';
					if(! $delivery_summ_empty)
					{
						$delivery_summ = $this->diafan->filter($_SESSION, "float", "cart_delivery_summ");
					}
				}
				else
				{
		...
		if(isset($_SESSION["cart_delivery_summ"]))
		{
			unset($_SESSION["cart_delivery_summ"]);
		}
		if(isset($_SESSION["cart_delivery_info"]))
		{
			unset($_SESSION["cart_delivery_info"]);
		}
