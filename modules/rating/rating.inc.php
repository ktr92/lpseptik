...
class Rating_inc extends Model
{
	 ...
	 * @return string
	 */
	function get($element_id = 0, $module_name = '', $element_type = 'element', $site_id = 0)
	{
		if(! $module_name)
		{
		...
		if(! empty($this->cache["prepare"]))
		{
			...
			foreach ($rows as $row)
			{
				$this->cache["rating"][$row["module_name"]][$row["element_type"]]['e'.$row["element_id"]] = round($row["rating"]);
			}
		...
		$this->result["rating"] = $this->cache["rating"][$module_name][$element_type]['e'.$element_id];
