...
class Menu_admin extends Frame_admin
{
	...
	public function save_variable_access()
	{
		if ($_POST["element_id"])
		{
			$element = DB::query_fetch_array("SELECT * FROM {%h} WHERE id=%d LIMIT 1", $this->diafan->table_element_type($_POST["module_name"]), $_POST["element_id"]);
		}
		elseif ($_POST["module_cat_id"])
		{
			$element = DB::query_fetch_array("SELECT access FROM {%h_category} WHERE id=%d LIMIT 1", $_POST["module_name"], $_POST["module_cat_id"]);
		}
		else
		{
			$element = DB::query_fetch_array("SELECT access, date_start, date_finish FROM {site} WHERE id=%d LIMIT 1", $_POST["site_id"]);
		}
