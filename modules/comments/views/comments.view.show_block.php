			...
			if($param["type"] == "attachments")
			{
				foreach ($param["value"] as $a)
				{
					if ($a["is_image"])
					{
						...
						{
							echo ' <a href="'.$a["link"].'" rel="prettyPhoto[gallery'.$row["id"].'comments]" class="block-row-img"><img class="avatar" src="'.$a["link_preview"].'"></a> <a href="'.$a["link"].'" rel="prettyPhoto[gallery'.$row["id"].'comments_link]">'.$a["name"].'</a>';
						}
						else
						{
