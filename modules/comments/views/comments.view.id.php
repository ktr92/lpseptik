		...
		if($param["type"] == "attachments")
		{
			foreach ($param["value"] as $a)
			{
				if ($a["is_image"])
				{
					...
					{
						echo ' <a href="'.$a["link"].'" rel="prettyPhoto[gallery'.$result["id"].'comments]"><img src="'.$a["link_preview"].'"></a> <a href="'.$a["link"].'" rel="prettyPhoto[gallery'.$result["id"].'comments_link]">'.$a["name"].'</a>';
					}
					else
					{
...
{
	echo '<div class="comments_level comments'.$result["id"].'_result">'.$this->get('list', 'comments', $result["children"]).'</div>';
}
else
{
