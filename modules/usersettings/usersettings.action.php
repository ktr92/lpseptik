...
class Usersettings_action extends Action
{
	...
	*/
	...
	public function edit()
	{
		...
		$where_param_role_rel = $this->get_where_param_role_rel($this->diafan->_users->role_id);
		...
		DB::query("UPDATE {users} SET name='%h', mail='%h', phone='%h', lang_id=%d, fio='%h' WHERE id=%d",
			  $_POST["name"], $_POST["mail"], (! empty($_POST["phone"]) ? $phone : ''), !empty($_POST["lang_id"]) ? $_POST["lang_id"] : 0,
			  $_POST["fio"], $this->diafan->_users->id
		);

		if(in_array('subscribtion', $this->diafan->installed_modules))
		{
			if(! $this->diafan->_route->id_module('subscribtion') && $this->diafan->configmodules('subscribe_in_registration', 'subscribtion'))
			{
				DB::query("UPDATE {subscribtion_emails} SET act='%d' WHERE mail='%s'", (empty($_POST['subscribe']) ? 0 : 1), $this->diafan->_users->mail);
			}
			// при смене e-mail, меняем его в списке рассылки
			if($this->diafan->_users->mail != $_POST["mail"])
			{
				DB::query("UPDATE {subscribtion_emails} SET mail='%s' WHERE mail='%s'", $_POST['mail'], $this->diafan->_users->mail);
			}
			if($_POST["phone"])
			{
				$phone = preg_replace('/[^0-9]+/', '', $_POST["phone"]);				
				...
				{
					DB::query("UPDATE {subscribtion_phones} SET phone='%s' WHERE phone='%s'", $phone, $this->diafan->_users->phone);
				}						
			}
		}
