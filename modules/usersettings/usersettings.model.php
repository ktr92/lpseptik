...
class Usersettings_model extends Model
{
	...
	public function form()
	{
		...
		{
			$this->result['link_subscribtion'] = BASE_PATH_HREF.$this->diafan->_route->module("subscribtion").'?action=edit&mail='.$this->diafan->_users->mail;
			if($code = DB::query_result("SELECT code FROM {subscribtion_emails} WHERE mail='%s' AND trash='0'", $this->diafan->_users->mail))
			{
		...
		else
		{
			...
			{
				$this->result['use_subscribtion'] = $this->diafan->configmodules('subscribe_in_usersettings', 'subscribtion');
				$this->result["is_subscribe"] = DB::query_result("SELECT id FROM {subscribtion_emails} WHERE mail='%s' AND trash='0' AND act='1' LIMIT 1", $this->diafan->_users->mail);
			}
		}
