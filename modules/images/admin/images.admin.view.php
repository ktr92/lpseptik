		...
		foreach ($rows as $row)
		{
			...
			<div class="images_actions'.($k == 1 ? ' first' :'').'" element_id="'.$element_id.'" image_id="'.$row["id"].'">
				<input type="checkbox" name="image_check">'
			       .(! empty($variation_folder) ? '<a href="'.BASE_PATH.USERFILES.'/'.$module_name.'/'.$variation_folder.'/'.($row["folder_num"] ? $row["folder_num"].'/' : '').$row["name"].'" class="image" rel="prettyPhoto[galleryshop]">' : '')
