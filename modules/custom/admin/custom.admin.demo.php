...
class Custom_admin_demo extends Diafan
{
			...
			foreach($tpl as $table => $config)
			{
				...
				switch($table)
				{
					...
					case 'site_blocks':
						...
						foreach($result[$table] as $row)
						{
							$install_sql .= "UPDATE {site_blocks} SET [name]='".str_replace("'", "\\'", $row["name"][0])."', [text]='".str_replace("'", "\\'", $row["text"][0])."', [act]='1', title_no_show='".($row["title_no_show"] ? 1 : 0)."' WHERE id=".$row["id"].";\n";
						}
	...
	{
		if($row["element_type"] == 'param' && ! empty($row["type"]) && in_array($row["type"], array('select', 'multiple')))
		{
