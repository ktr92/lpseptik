...
class Wishlist_inc extends Diafan
{
	...
	public function get_count()
	{
		...
		{
			$count = DB::query_result("SELECT SUM(count) FROM {shop_wishlist} WHERE user_id='%s'", $this->diafan->_users->id);
			return (empty($count) ? 0 : $count);
		}
		else
	    {
	    	$count = DB::query_result("SELECT SUM(count) FROM {shop_wishlist} WHERE session_id='%s'", session_id());
	    	return (empty($count) ? 0 : $count);
	    }
	}
