	...
	private function set_price_count()
	{
		...
		if ($this->is_field("price"))
		{
			if($this->is_field("id"))
			{
				...
				DB::query("DELETE FROM {shop_price_image_rel} WHERE price_id IN (SELECT price_id FROM {shop_price} WHERE good_id=%d)", $this->id);
			...
			foreach ($price_value as $row)
			{
				...
				$this->diafan->_shop->price_insert($this->id, $row["price"], $row["old_price"], $row["count"], $row["params"], $row["currency"]);
