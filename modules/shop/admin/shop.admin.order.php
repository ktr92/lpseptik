...
class Shop_admin_order extends Frame_admin
{
	...
	{
		$res = $this->diafan->filter($_GET, 'int', "filter_user_id");
		if($res)
		{
			$this->diafan->join .= " ";
			$this->diafan->join .= " ";
			$this->diafan->where .= " AND (user_id='". $this->diafan->filter($_GET, 'int', "filter_user_id")."')";
			$this->diafan->get_nav .= ($this->diafan->get_nav ? '&amp;' : '?') . 'filter_user_id='.$this->diafan->filter($_GET, 'url', "filter_user_id");
	...
	public function edit_variable_goods()
	{
		...
		if(! $this->diafan->is_new)
		{
			$rows = DB::query_fetch_all("SELECT a.id, a.[name], a.price, a.amount, s.id AS sid, s.summ FROM {shop_additional_cost} AS a LEFT JOIN {shop_order_additional_cost} AS s ON s.additional_cost_id=a.id AND s.order_id=%d WHERE a.trash='0' AND a.shop_rel='0'", $this->diafan->id);
		}
		else
		{
			$rows = DB::query_fetch_all("SELECT a.id, a.[name], a.price, a.amount FROM {shop_additional_cost} AS a WHERE a.trash='0' AND a.shop_rel='0'");
		}
		if($rows)
		{
			echo '<li class="item">
...
<div class="num"><input name="discount_summ" value="'
	...
	public function save_variable_delivery_id()
	{
		...
		{
			$delivery_summ = $this->diafan->filter($_POST, "integer", "delivery_summ");
