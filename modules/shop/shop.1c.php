...
class Shop_1c extends Diafan
{
	...
	private function sale_query()
	{
		...
		foreach ($rows as $row)
		{
			...
			{
				$payment_name = DB::query_result("SELECT p.[name] FROM {payment} AS p INNER JOIN {payment_history} AS h ON h.payment_id=p.id WHERE h.element_id=%d AND h.module_name='cart'", $row["id"]);
				$s1_3 = $s1_2->addChild("ЗначениеРеквизита" );
				$s1_3->addChild("Наименование", "Метод оплаты");
				$s1_3->addChild("Значение", $payment_name);
			}
		}
	...
	private function import_goods($xml, $site_id)
	{
		...
		foreach ($xml->Товары->Товар as $xml_product)
		{
			...
			{
				$row = DB::query_fetch_array("SELECT id, cat_id, article, [measure_unit], [name], [text], [anons], brand_id FROM {shop} WHERE import_id='%h' AND trash='0' LIMIT 1", $good_id_1c);
				...
				{
					if(! isset($_SESSION["1c_cats"][strval($xml_product->Группы->Ид)]))
					{
						$_SESSION["1c_cats"][strval($xml_product->Группы->Ид)] = DB::query_result("SELECT id FROM {shop_category} WHERE trash='0' AND import_id='%s'", $xml_product->Группы->Ид);
					}
				...
				if(! $id)
				{
					$id = DB::query("INSERT INTO {shop} ([name], [text], article, [measure_unit], import_id, cat_id, site_id, timeedit, brand_id, [act]) VALUES ('%h', '%s', '%h', '%h', '%h', %d, %d, %d, %d, '%d')", $xml_product->Наименование, $description, $xml_product->Артикул, $measure_unit, $good_id_1c, $cat_id, $site_id, time(), $brand_id, ($this->diafan->configmodules("1c_act", "shop") ? 1 : 0));
					if($cat_id)
					{
					...
					if(! empty($this->cache["params"][strval($xml_option->Ид)]))
					{
						...
						foreach ($xml_option->Значение as $xml_value)
						{
							...
							if(! empty($xml_value))
							{
								...
								{
									$val = $this->cache["params"][strval($xml_option->Ид)]["values"][$xml_value];
								}
								else
								{
									$val = $xml_value;
	...
	private function import_prices($xml)
	{
		...
		foreach ($xml->Предложения->Предложение as $xml_variant)
		{
			...
			if($variant_id_1c && ! $params && preg_match('/^'.preg_quote($row["name"._LANG], '/').' \((.*)\)$/', $xml_variant->Наименование, $m))
			{
				...
				else
				{
					...
					if($param_id = $this->cache["1_params"]['Тип'])
					{
						...
						if(! in_array($row["cat_id"], $this->cache["multiple_params"][$param_id]["cats"]))
						{
			...
			if(! empty($xml_variant->Количество))
			{
	...
	private function price_param($xml_product, $cat_id)
	{
		...
		if(isset($xml_product->ХарактеристикиТовара->ХарактеристикаТовара))
		{
			foreach ($xml_product->ХарактеристикиТовара->ХарактеристикаТовара as $xml_property)
			{
				...
				if(! isset($this->cache["1_params"][$name]))
				{
					$r = DB::query_fetch_array("SELECT id, required FROM {shop_param} WHERE [name]='%h' AND type='multiple' AND trash='0' LIMIT 1", $name);
					if(! $r)
					{
