...
class Shop_action extends Action
{
	...
	public function wait()
	{
		...
		$rows_param = DB::query_fetch_all(
				"SELECT p.[name], p.id, e.value".$this->diafan->_languages->site." AS value FROM {shop_param} AS p"
				." INNER JOIN {shop_param_element} AS e ON e.element_id=%d AND e.param_id=p.id"
	...
	public function add_coupon()
	{
		...
		$coupon_id = DB::query_result("SELECT c.id FROM {shop_discount} AS d"
		...
		$discount = DB::query_fetch_array("SELECT d.id, c.count_use, c.used, d.date_finish, c.id as coupon_id FROM {shop_discount} AS d"
			." INNER JOIN {shop_discount_coupon} AS c ON c.discount_id=d.id"
			." WHERE d.act='1' AND d.trash='0' AND c.coupon='%s' LIMIT 1", $_POST["coupon"]);
		if (empty($discount))
		{
			$this->result["errors"][0] = $this->diafan->_('Вы ввели ошибочный код купона.', false);
			return;
		...
		{
			$this->result["errors"][0] = $this->diafan->_('Купон использован кем-то другим.', false);
			return;
		}
		if($discount["date_finish"] && $discount["date_finish"] < time())
		{
			$this->result["errors"][0] = $this->diafan->_('Время действия купона истекло.', false);
			return;
		}
