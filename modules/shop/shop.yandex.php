		...
		foreach ($this->sites as $site_id)
		{
			...
			while(1==1)
			{
				...
				foreach ($rows as $row)
				{
					...
					$imgs = DB::query_fetch_all("SELECT id, name, folder_num FROM {images} WHERE module_name='shop' AND"
									." element_id='%d' AND element_type='element' AND trash='0' ORDER BY sort ASC", $row["id"]);
