...
class Shop_model extends Model
{
	...
	public function list_()
	{
		...
		if ( ! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->_site->module))
		{
			...
			if($this->diafan->_route->brand)
			{
				...
				$this->result["url_brand"] = $brand["url_brand"];
				$this->result["country"] = $brand["country"];
	...
	private function list_brand_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, [title_meta], [act], theme, view, url_brand, country, noindex FROM {shop_brand}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
		.(! $this->is_admin() ? " AND [act]='1'" : '')
	...
	public function show_brand($count, $site_ids, $cat_ids, $sort, $images, $images_variation)
	{
		...
		if ($sort == "rand" || ! $this->result = $this->diafan->_cache->get($cache_meta, "shop"))
		{
			...
			foreach ($rands as $rand)
			{
				$rows = DB::query_fetch_all("SELECT e.id, e.[name], e.[text], e.country, e.site_id
	...
	public function elements(&$rows, $function = 'list', $images_config = '')
	{
		...
		{
			$brands = DB::query_fetch_all("SELECT id, [name], url_brand, country, site_id FROM {shop_brand} WHERE trash='0' AND [act]='1' AND id IN (%s)", implode(",", array_keys($this->cache["prepare_brand"])));
			foreach($brands as $b)
			{
