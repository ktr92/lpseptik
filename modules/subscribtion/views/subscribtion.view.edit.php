...
if(! empty($result['cats']) || empty($result["act"]))
{
	...
	if(! empty($result['cats']))
	{
		...
		{
			echo str_repeat('&nbsp;', 4*$cat["level"]).'<input type="checkbox" name="cat_ids[]" value="'.$cat["id"].'" id="subscribtion_p'.$cat["id"].'" '.(! in_array($cat["id"], $result["cats_unrel"]) ? ' checked' : '').'><label for="subscribtion_p'.$cat["id"].'">'.$cat["name"].'</label></br>';
		}
	}
	echo '
