...
class Subscribtion_admin_phones extends Frame_admin
{
	...
	private function form_upload()
	{
		...
		<div class="box box_half box_height box_right">
			...
			<a href="'.BASE_PATH.'subscribtion/export/phones/?'.rand(0, 999999).'" class="file-load">
	...
	{	
		...
		{
			DB::query("TRUNCATE TABLE {subscribtion_phones}");
		}
		...
		if(empty($_POST["delete_old"]))
		{
			$oldphones = DB::query_fetch_key_value("SELECT * FROM {subscribtion_phones} WHERE trash='0'", "phone", "id");
		}
		...
		foreach ($newphones as $s)
		{
			...
			{
				DB::query("UPDATE {subscribtion_phones} SET act='1', name='%s' WHERE id=%d", $name, $oldphones[$phone]);
				$count_upd++;
			}
			else
			{
				DB::query("INSERT INTO {subscribtion_phones} (act, name, phone, created) VALUES ('1', '%s', '%s', %d)", $name, $phone, time());
				$count_add++;
			}
		}
