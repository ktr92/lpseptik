...
foreach ($result["rows"] as $row)
{
	...
	if (! empty($row["img"]))
	{
		...
		foreach ($row["img"] as $img)
		{
			...
			{
				case 'animation':
					echo '<a href="'.BASE_PATH.$img["link"].'" rel="prettyPhoto[gallery'.$row["id"].'files]" class="block-row-img">';
					break;
				case 'large_image':
		...
		echo '<div class="block-text">';
			...
			if (! empty($row["files"]))
			{
				foreach ($row["files"] as $f)
				{
					...
					echo '<a href="'.$f["link"].'" class="button solid download">'.$this->diafan->_('Скачать').'</a>';						
				}
			}
