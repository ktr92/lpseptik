...
class Ab_admin_param extends Frame_admin
{
	...
	public function edit_variable_category()
	{
		...
		$as_pages = DB::query_fetch_key_value("SELECT id, [name] FROM {site} WHERE trash='0' AND module_name='ab'", "id", "name");

		echo '
		<div class="unit">
			<div class="infofield">'.$this->diafan->_('Категория').$this->diafan->help().'</div>';

		$rows = DB::query_fetch_all("SELECT id, [name], parent_id, site_id FROM {ab_category} WHERE trash='0' ORDER BY sort ASC");
		foreach ($rows as $row)
		{
		...
		foreach ($as_pages as $site_id => $name)
		{
