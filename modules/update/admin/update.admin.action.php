...
class Update_admin_action extends Action_admin
{
	...
	private function update()
	{
		...
		if(empty($this->result["redirect"]))
		{
			...
			{
				$this->result["messages"] = '<div class="ok">'.$this->diafan->_('Обновлений нет.').'</div>';
			}
		}
	...
	private function download()
	{
		...
		if($id)
		{
			File::copy_file('http://user.diafan.ru/file/update/'.$_POST["id"].'/'.$_POST["hash"], 'return/'.$id.'.zip');

			DB::query("INSERT INTO {update_return} (id, name, created, `text`, `hash`, current) VALUES (%d, 'Обновление', %d, '%h', '%h', '0')", $_POST["id"], time(), $_POST["text"], $_POST["hash"]);
