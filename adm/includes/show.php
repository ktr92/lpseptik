...
class Show_admin extends Diafan
{
	 ...
	 * @return void
	 */
	public function group_action_panel()
	{
		...
		if($this->diafan->group_action)
		{
			...
			{
				$html .= '<option value="'.$action.'"'.($value == $action ? ' selected' : '').''.(! empty($row["module"]) ? ' module="'.$row["module"].'"' : '').'>'.$this->diafan->_($row["name"]).'</option>';
			}
		}
		...
		if($this->diafan->variables_filter)
		{
			$html .= '
